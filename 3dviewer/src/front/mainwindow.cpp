...
void MainWindow::showFileDialog() {
  ...
  if (!filePath.empty()) {
    ...
    auto vert = ptrControl->getVertixes().size();
    auto edge = ptrControl->getEdges().size();
...
void MainWindow::connectSlidersandSpinBoxes() {
  ...
  connect(ui->slider_scaling, &QSlider::valueChanged, glWidget,
          &Viewport::doScale);
  connect(ui->spinBox_7, &QSpinBox::valueChanged, glWidget, &Viewport::doScale);
  // rotate horizont
  ...
  //           rotate_lamda);
  // rotate vertical
  ui->slider_vertical_rotation->setMinimum(-90);
  ...
  connect(ui->spinBox_vertical_rotation, &QSpinBox::valueChanged, rotate_lamda);
  ...
  connect(ui->spinBox, &QSpinBox::valueChanged, ui->horizontalSlider_move_x,
          &QSlider::setValue);
  //   connect(ui->spinBox, &QSpinBox::valueChanged, move_lamda);
  // move y
  ...
  connect(ui->spinBox_3, &QSpinBox::valueChanged, ui->horizontalSlider_move_y,
          &QSlider::setValue);
  //   connect(ui->spinBox_3, &QSpinBox::valueChanged, move_lamda);
  // move z
  ...
  connect(ui->spinBox_4, &QSpinBox::valueChanged, ui->horizontalSlider_move_z,
          &QSlider::setValue);
  //   connect(ui->spinBox_4, &QSpinBox::valueChanged, move_lamda);
}
